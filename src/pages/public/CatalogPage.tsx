...
import type { Category, Gender, Size, ProductFilters, Product } from '../../types';
...
function FilterSection({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className="border-b border-border py-4">
      ...
      >
        {title}
        {open ? <ChevronUp className="h-3 w-3" strokeWidth={1.5} /> : <ChevronDown className="h-3 w-3" strokeWidth={1.5} />}
      </button>
  ...
  const SidebarContent = () => (
    <div className="flex flex-col">
      <div className="flex items-center justify-between border-b border-border pb-4">
        <span className="text-[10px] uppercase tracking-widest text-foreground">Filters</span>
        {hasActiveFilters && (
          <button onClick={clearFilters} className="text-[10px] uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors">
            Clear all
          </button>
        )}
      </div>
      ...
      <FilterSection title="Gender">
        <ul className="flex flex-col gap-2.5">
          {GENDERS.map((g) => (
            <li key={g.value}>
              <button
                onClick={() => updateFilter('gender', filters.gender === g.value ? undefined : g.value)}
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between border-b border-border py-8">
          ...
          <div className="flex items-center gap-4">
            {data && <span className="text-xs text-muted-foreground">{data.meta.total} items</span>}
            <button
          ...
          <div className="flex-1">
            ...
            {!isLoading && !isError && data?.data.length === 0 && (
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <button onClick={clearFilters} className="mt-4 text-xs uppercase tracking-widest text-foreground underline underline-offset-2">
                  Clear filters
                </button>
              </div>
            )}
