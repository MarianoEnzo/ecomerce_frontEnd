...
function ProductModal({
  ...
  const inputClass =
    "border-b border-border bg-transparent pb-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-foreground focus:outline-none transition-colors w-full";
  const labelClass =
    "text-[10px] uppercase tracking-widest text-muted-foreground";
  const selectClass =
    "border-b border-border bg-background pb-2 text-sm text-foreground focus:border-foreground focus:outline-none w-full";
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() =>
                  append({ size: "M", stock: 0, colorId: 1, imageUrl: "" })
                }
            ...
            {fields.map((field, index) => (
              <div
                key={field.id}
                className="grid grid-cols-4 gap-3 border-b border-border pb-4"
              >
                <div className="flex flex-col gap-1.5">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-border">
                  {["Name", "Category", "Price", "Variants", "Status", ""].map(
                    (h) => (
                      <th
                        key={h}
                ...
                {data.data.map((product) => (
                  ...
                  >
                    ...
                    <td className="py-4 pr-6">
                      <span
                        className={`text-[10px] uppercase tracking-widest ${product.isActive ? "text-green-600" : "text-muted-foreground"}`}
                      >
